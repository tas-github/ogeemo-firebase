...
import { Settings, Plus, Trash2, PlusCircle, MoreVertical, BookOpen, Pencil, LoaderCircle } from "lucide-react";
  ...
  return (
    <>
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col">
          <header className="text-center mb-6 w-full mx-auto">
            <h1 className="text-3xl font-bold font-headline text-primary">General Ledger</h1>
      ...
      <Dialog open={isTransactionDialogOpen} onOpenChange={setIsTransactionDialogOpen}>
        <DialogContent>
          <DialogHeader className="text-center sm:text-center"><DialogTitle className="text-2xl text-primary font-bold">{transactionToEdit ? 'Edit Transaction' : `Post New ${newTransactionType === 'income' ? 'Income' : 'Expense'}`}</DialogTitle><DialogDescription>Select transaction type and fill details.</DialogDescription></DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="tx-company-gl" className="text-right">Company <span className="text-destructive">*</span></Label><div className="col-span-3"><Select value={newTransaction.company} onValueChange={(value) => setNewTransaction(prev => ({...prev, company: value}))}><SelectTrigger id="tx-company-gl" className="w-full"><SelectValue placeholder="Select a company" /></SelectTrigger><SelectContent>{companies.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div></div>
            ...
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="tx-category-gl" className="text-right">{newTransactionType === 'income' ? 'Income Type' : 'Category'} <span className="text-destructive">*</span></Label><div className="col-span-3"><Select value={newTransactionType === 'income' ? newTransaction.incomeType : newTransaction.category} onValueChange={(value) => setNewTransaction(prev => ({...prev, [newTransactionType === 'income' ? 'incomeType' : 'category']: value}))}><SelectTrigger id="tx-category-gl" className="w-full"><SelectValue placeholder="Select a category" /></SelectTrigger><SelectContent>{(newTransactionType === 'income' ? incomeTypes : expenseCategories).map(cat => <SelectItem key={cat} value={cat}>{cat}</SelectItem>)}</SelectContent></Select></div></div>
