...
export function TextEditorView() {
  ...
  const handleSave = useCallback(async (isAutoSave = false) => {
    if (!file || !editorRef.current || isSaving) return;
    ...
    if (!isAutoSave && newContent === (file.content || '')) {
    ...
    try {
        ...
        const fileBlob = new Blob([newContent], { type: file.type || 'text/plain;charset=utf-8' });

        await uploadBytes(storageFileRef, fileBlob);
        
        await updateFile(file.id, { size: fileBlob.size });
        
        setFile(prevFile => prevFile ? { ...prevFile, content: newContent, size: fileBlob.size } : null);
        if (!isAutoSave) {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
        if (editorRef.current && file && editorRef.current.innerHTML !== (file.content || '')) {
            handleSave(true);
            const message = "You have unsaved changes. They are being saved now.";
    ...
    <div className="p-4 sm:p-6 h-full flex flex-col">
        <header className="flex justify-between items-center mb-4">
            ...
            <div className="text-center">
                <h1 className="text-xl font-bold font-headline text-primary truncate" title={file?.name}>{file?.name || 'Untitled Document'}</h1>
