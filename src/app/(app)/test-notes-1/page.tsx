               ...
               <div className="flex-1 overflow-y-auto">
                {filesForSelectedFolder.map(file => (
                    <div key={file.id} className={cn("flex items-center p-2 rounded-md cursor-pointer group", selectedFile?.id === file.id && "bg-accent")}>
                        ...
                        <div className="opacity-0 group-hover:opacity-100 flex items-center">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleStartRenameFile(file)}><Edit className="h-4 w-4"/></Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setFileToDelete(file)}><Trash2 className="h-4 w-4 text-destructive"/></Button>
                        </div>
                    </div>
                ))}
              </div>
